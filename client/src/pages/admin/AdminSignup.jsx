...
export default function AdminSignup() {
  ...
  useEffect(() => {
    api.get("/auth/admin/status").then(({ data }) => setAdminExists(data.adminExists)).catch(() => {});
  }, []);
  ...
  async function submit(e) {
    ...
    try {
      ...
      navigate(`/admin/verify-otp?email=${encodeURIComponent(form.email)}${data.devOtp ? `&otp=${encodeURIComponent(data.devOtp)}` : ""}`);
    } catch (error) {
    ...
    <main className="mx-auto flex min-h-[70vh] max-w-md items-center px-4">
      <form onSubmit={submit} className="panel w-full space-y-4 p-6">
        <div>
          ...
          <p className="text-sm text-gray-600">Only the first verified Gmail account can become admin.</p>
        </div>
        {adminExists ? (
          <div className="rounded-md bg-red-50 p-4 text-sm font-semibold text-red-700">
            Admin profile already exists. Use admin login.
          </div>
        ) : (
          <>
