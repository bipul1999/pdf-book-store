...
export default function Library() {
  ...
  const loadLibrary = () => {
    ...
    api.get("/users/library")
      ...
      .catch((requestError) => setError(requestError.response?.data?.message || "Your library could not be loaded. Please try again."))
      .finally(() => setLoading(false));
  };
  useEffect(() => { loadLibrary(); }, []);
  return <main className="store-page mx-auto w-full max-w-6xl px-4 py-5 sm:py-9"><h1 className="mb-3 text-2xl font-black sm:text-3xl">My Library</h1><p className="mb-5 text-sm font-semibold leading-6 text-gray-600">Your purchased book always opens its latest PDF version. Reading access remains available until the date set for your purchase.</p>{loading ? <div className="panel p-8 text-center font-bold text-gray-600">Loading your purchased books…</div> : error ? <div className="panel p-8 text-center text-gray-700"><p className="font-bold text-red-600">{error}</p><button className="btn-primary mt-4" onClick={loadLibrary} type="button">Try again</button></div> : <><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{books.map((book) => <article className="panel grid grid-cols-[84px_1fr] gap-3 p-3 transition hover:-translate-y-0.5 hover:shadow-[0_18px_42px_rgba(120,53,15,.12)]" key={book._id}><img src={book.coverImage} className="h-28 w-20 rounded-xl bg-orange-50 object-contain p-1" alt={book.title} /><div className="min-w-0"><h3 className="line-clamp-2 font-black leading-snug">{book.title}</h3><p className="text-sm text-gray-600">{book.author}</p>{book.accessExpiresAt && <p className="mt-1 text-xs font-bold text-orange-600">{accessLabel(book.accessExpiresAt)}</p>}<Link className="btn-primary mt-3 w-full sm:w-auto" to={`/dashboard/library/${book._id}/read`}><BookOpen size={16} /> Read PDF</Link></div></article>)}</div>{!books.length && <div className="panel p-8 text-center text-gray-600"><p className="font-bold">No purchased books are linked to this account yet.</p><Link className="btn-primary mt-4" to="/books">Browse books</Link></div>}</>}</main>;
}
